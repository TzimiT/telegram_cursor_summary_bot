...
**Проблема:** Функции определены в `scripts/get_users.py` и `scripts/run_daily.py` с одинаковой логикой.
...
**Проблема:** В `scripts/get_users.py` используются синхронные операции с файлами в async-функциях.
...
**Проблема:** В `src/news_bot_part.py` и `scripts/run_daily.py` создается новый `Bot` при каждом вызове.

**Решение:** Singleton или передача через параметры:
```python
# В scripts/run_daily.py
